...
The worked example makes this concrete. Closure's principals are org-scoped — humans carry roles (owner, admin, builder, viewer), IDE sessions run on capability-scoped keys that an administrator can revoke, and **authorization is expressed as Policy documents on the graph** (`schema:policy`): Principal × Action × Resource × Condition → Effect, evaluated by one decision point. Role×pillar presets still seed those Policies so the familiar shelf (experiences, workflows, knowledge, integrations — none / read / write) remains the default; orgs may refine to individual objects, environments (Personal / Dev / Test / Prod), and tools because permissions are data. Integrity validation (SIV) is a separate layer — *whether* a change is well-formed — not *who* may attempt it. A builder who may edit workflows but not touch integrations is a Policy row, not a memo.
...
## Dual containment — agents and humans

The industry’s fear is agents going rogue. That fear is correct and incomplete. Humans go rogue too — silent Prod edits, rubber-stamp approvals, policy widened for convenience, credentials shared “just this once.” A substrate that only gates machines invites the oldest bypass: a privileged person. A substrate that only gates people invites agent sprawl under borrowed keys.

Comprehensive governance is **dual containment**. The same Policy Decision Point evaluates every consequential request, whether the principal is human, agent, or service. Capabilities differ by design — agents typically cannot move Prod pins; humans with owner roles can, but under N-approvers, change windows, and quality gates expressed as Policy conditions. Neither kind skips the PDP via an environment flag. Break-glass, when it exists, is itself a time-boxed Policy with mandatory audit — not a hole in the wall. Part V (Appendix 23) states this as mathematics: one $\mathsf{PDP}$, effect algebra deny > require_human > allow, write-class fail closed, meta-governance so Policies govern Policies.

Authorization lives on the graph as `schema:policy` documents. Integrity validation (SIV) remains a separate question — *whether* a change is well-formed — so a clever agent cannot launder a denied action by producing pretty JSON, and a well-typed change from a denied principal still does not commit.

## The oldest requirement

Step back and look at the assembled machinery: distinct identities for every actor; Policies on the graph evaluated by one decision point; changes that exist as reviewable artifacts before they take effect; versioned history and pointer-move rollback; separated environments with immutable releases; approval gates scaling to dual control; secrets sealed away from the actors that use them.

There is nothing novel in that list. That is the point of the list.

Every item is something enterprises have demanded of *human* change for decades — the doctrine that auditors examine and regulations encode. The machinery is old and proven. What is new is only its reach: because the product is data, the doctrine now covers actors that are not people — and binds the people with the same decision point. The change-managed employee and the change-managed agent pass through the same states, leave the same evidence, and roll back the same way. The security architect from the opening scene finally receives answers instead of adjectives. *What exactly can this thing do?* — simulate the Policy decision for its principal. *How do we undo it?* — re-point the version. *Who approved it?* — the release record, same as for people.
